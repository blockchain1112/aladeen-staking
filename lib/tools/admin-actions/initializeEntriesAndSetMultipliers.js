...
const handler = async (connection, wallet, args) => {
    const { stakePoolId, initEntries, fungible, metadataRules, distributorId } = args;
    ...
    const stakeEntryIds = await Promise.all(initEntries.map((e) => (0, pda_2.findStakeEntryId)(wallet.publicKey, stakePoolId, e.mintId, fungible)));
    ...
    for (let i = 0; i < batchedChunks.length; i++) {
        ...
        await Promise.all(chunk.map(async (entries, c) => {
            ...
            for (let j = 0; j < entries.length; j++) {
                ...
                try {
                    const stakeEntryId = (0, pda_2.findStakeEntryId)(wallet.publicKey, stakePoolId, mintId, fungible);
