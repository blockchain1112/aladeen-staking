...
const withRemainingAccountsForKind = async (transaction, connection, wallet, rewardDistributorId, kind, rewardMint, isClaimRewards, createRewardDistributorMintTokenAccount = false) => {
    switch (kind) {
        ...
        case constants_1.RewardDistributorKind.Treasury: {
            const rewardDistributorRewardMintTokenAccountId = createRewardDistributorMintTokenAccount
                ? await (0, common_1.withFindOrInitAssociatedTokenAccount)(transaction, connection, rewardMint, rewardDistributorId, wallet.publicKey, true)
                : !createRewardDistributorMintTokenAccount
                    ? await (0, common_1.findAta)(rewardMint, rewardDistributorId, true)
                    : await (0, common_1.withFindOrInitAssociatedTokenAccount)(transaction, connection, rewardMint, rewardDistributorId, wallet.publicKey, true);
