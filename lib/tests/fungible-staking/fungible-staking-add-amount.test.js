...
describe("Create stake pool", () => {
    ...
    it("Stake another half", async () => {
        ...
        const stakeEntryDataBefore = await (0, accounts_2.getStakeEntry)(provider.connection, stakeEntryId);
        const transaction = await (0, src_1.stake)(provider.connection, provider.wallet, {
            stakePoolId: stakePoolId,
            originalMintId: originalMintId,
            userOriginalMintTokenAccountId: originalMintTokenAccountId,
        ...
        await (0, utils_2.executeTransaction)(provider.connection, transaction, provider.wallet);
        const stakeEntryDataAfter = await (0, accounts_2.getStakeEntry)(provider.connection, stakeEntryId);
        const userOriginalMintTokenAccountId = await (0, common_1.findAta)(originalMintId, provider.wallet.publicKey, true);
        const stakeEntryOriginalMintTokenAccountId = await (0, common_1.findAta)(originalMintId, stakeEntryDataAfter.pubkey, true);
        const checkUserOriginalTokenAccount = await (0, spl_token_1.getAccount)(provider.connection, userOriginalMintTokenAccountId);
        expect(Number(checkUserOriginalTokenAccount.amount)).toEqual(0);
        const checkStakeEntryOriginalMintTokenAccount = await (0, spl_token_1.getAccount)(provider.connection, stakeEntryOriginalMintTokenAccountId);
        expect(Number(checkStakeEntryOriginalMintTokenAccount.amount)).toEqual(stakingAmount);
        // stake seconds increased
        expect(stakeEntryDataAfter.parsed.totalStakeSeconds.toNumber()).toBeGreaterThan(stakeEntryDataBefore.parsed.totalStakeSeconds.toNumber() + 4);
    });
    it("Unstake", async () => {
...
//# sourceMappingURL=fungible-staking-add-amount.test.js.map