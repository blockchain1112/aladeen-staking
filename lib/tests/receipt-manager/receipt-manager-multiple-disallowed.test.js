...
describe("Receipt manages multiple with disallowlist", () => {
    ...
    const requiredStakeSeconds = new anchor_1.BN(0);
    ...
    it("Create Reward Receipt Manager", async () => {
        ...
        expect(receiptManagerData.parsed.name.toString()).toEqual(receiptManagerName1);
    ...
    it("Create Reward Receipt Manager", async () => {
        ...
        const receiptManagerData = await (0, accounts_2.getReceiptManager)(provider.connection, receiptManagerId);
        const [payamentManagerId] = await (0, pda_1.findPaymentManagerAddress)(receiptManager_1.RECEIPT_MANAGER_PAYMENT_MANAGER_NAME);
        expect(receiptManagerData.parsed.name.toString()).toEqual(receiptManagerName2);
        expect(receiptManagerData.parsed.paymentManager.toString()).toEqual(payamentManagerId.toString());
        expect(receiptManagerData.parsed.authority.toString()).toEqual(provider.wallet.publicKey.toString());
        expect(receiptManagerData.parsed.paymentMint.toString()).toEqual(paymentMint.toString());
        expect(receiptManagerData.parsed.stakePool.toString()).toEqual(stakePoolId.toString());
    ...
    it("Init Reward Receipt, first receipt manager", async () => {
        const transaction = new web3_js_1.Transaction();
        const receiptManagerId = (0, pda_2.findReceiptManagerId)(stakePoolId, receiptManagerName1);
        const stakeEntryId = (0, pda_3.findStakeEntryId)(provider.wallet.publicKey, stakePoolId, originalMintId, false);
    ...
    it("Init Reward Receipt, second receipt manager", async () => {
        const transaction = new web3_js_1.Transaction();
        const receiptManagerId = (0, pda_2.findReceiptManagerId)(stakePoolId, receiptManagerName2);
        const stakeEntryId = (0, pda_3.findStakeEntryId)(provider.wallet.publicKey, stakePoolId, originalMintId, false);
        const receiptEntryId = (0, pda_2.findReceiptEntryId)(stakeEntryId);
        const [, rewardReceiptId] = await (0, transaction_2.withInitRewardReceipt)(transaction, provider.connection, provider.wallet, {
            receiptManagerId: receiptManagerId,
    ...
    it("Set requires authorization to true for second receipt manager", async () => {
        const transaction = new web3_js_1.Transaction();
        const receiptManagerId = (0, pda_2.findReceiptManagerId)(stakePoolId, receiptManagerName2);
        const beforeReceiptManagerData = await (0, accounts_2.getReceiptManager)(provider.connection, receiptManagerId);
        expect(beforeReceiptManagerData.parsed.requiresAuthorization).toBeFalsy();
        await (0, transaction_2.withUpdateReceiptManager)(transaction, provider.connection, provider.wallet, {
            name: receiptManagerName2,
            stakePoolId: stakePoolId,
            authority: beforeReceiptManagerData.parsed.authority,
            requiredStakeSeconds: beforeReceiptManagerData.parsed.requiredStakeSeconds,
        ...
        await (0, utils_2.executeTransaction)(provider.connection, transaction, provider.wallet);
        const afterReceiptManagerData = await (0, accounts_2.getReceiptManager)(provider.connection, receiptManagerId);
    ...
    it("Set reward receipt auth to false, second receipt manager", async () => {
        ...
        const receiptManagerId = (0, pda_2.findReceiptManagerId)(stakePoolId, receiptManagerName2);
        const receiptEntryId = (0, pda_2.findReceiptEntryId)(stakeEntryId);
        const receiptId = (0, pda_2.findRewardReceiptId)(receiptManagerId, receiptEntryId);
        const beforeReceiptData = await (0, accounts_2.getRewardReceipt)(provider.connection, receiptId);
        expect(beforeReceiptData.parsed.allowed).toBeTruthy();
        await (0, transaction_2.withSetRewardReceiptAllowed)(transaction, provider.connection, provider.wallet, {
            auth: false,
            receiptManagerId: receiptManagerId,
            rewardReceiptId: receiptId,
        });
        await (0, utils_2.executeTransaction)(provider.connection, transaction, provider.wallet);
        const afterReceiptData = await (0, accounts_2.getRewardReceipt)(provider.connection, receiptId);
...
//# sourceMappingURL=receipt-manager-multiple-disallowed.test.js.map