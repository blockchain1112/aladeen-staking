...
exports.withSetRewardReceiptAllowed =
  exports.withCloseRewardReceipt =
  exports.withCloseReceiptEntry =
  exports.withCloseReceiptManager =
  exports.withClaimRewardReceipt =
  exports.withUpdateReceiptManager =
  exports.withInitRewardReceipt =
  exports.withInitReceiptEntry =
  exports.withInitReceiptManager =
    void 0;
...
const withInitReceiptManager = async (
  transaction,
  connection,
  wallet,
  params
  ...
  var _a;
  const receiptManagerId = (0, pda_1.findReceiptManagerId)(
    params.stakePoolId,
    params.name
  );
  ...
  const ix = await program.methods
    .initReceiptManager({
      ...
      paymentManager:
        params.paymentManagerId || constants_1.RECEIPT_MANAGER_PAYMENT_MANAGER,
      paymentRecipient: params.paymentRecipientId,
      requiresAuthorization: params.requiresAuthorization,
      maxClaimedReceipts:
        (_a = params.maxClaimedReceipts) !== null && _a !== void 0 ? _a : null,
    })
    .accounts({
...
const withInitReceiptEntry = async (
  transaction,
  connection,
  wallet,
  params
...
const withInitRewardReceipt = async (
  transaction,
  connection,
  wallet,
  params
) => {
  const rewardReceiptId = (0, pda_1.findRewardReceiptId)(
...
const withUpdateReceiptManager = async (
  transaction,
  connection,
  wallet,
  params
  ...
  const receiptManagerData = await (0, accounts_2.getReceiptManager)(
    connection,
    receiptManagerId
  );
  const program = (0, constants_1.receiptManagerProgram)(connection, wallet);
...
const withClaimRewardReceipt = async (
  transaction,
  connection,
  wallet,
  params
  ...
  const checkReceiptManager = await (0, common_1.tryGetAccount)(() =>
    (0, accounts_2.getReceiptManager)(connection, receiptManagerId)
  );
  ...
  const checkPaymentManager = await (0, common_1.tryGetAccount)(() =>
    (0, accounts_1.getPaymentManager)(
      connection,
      checkReceiptManager.parsed.paymentManager
    )
  );
  ...
  const feeCollectorTokenAccountId = await (0,
  ...
  const paymentRecipientTokenAccountId = await (0,
  common_1.withFindOrInitAssociatedTokenAccount)(
    transaction,
    connection,
    checkReceiptManager.parsed.paymentMint,
  ...
  await (0, transaction_1.withUpdateTotalStakeSeconds)(
    transaction,
    connection,
    wallet,
    {
...
const withCloseReceiptManager = async (
  transaction,
  connection,
  wallet,
  params
...
const withCloseReceiptEntry = async (
  transaction,
  connection,
  wallet,
  params
...
const withCloseRewardReceipt = async (
  transaction,
  connection,
  wallet,
  params
...
const withSetRewardReceiptAllowed = async (
  transaction,
  connection,
  wallet,
  params
