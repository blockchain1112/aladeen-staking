...
exports.withClaimStakeEntryFunds =
  exports.withInitUngrouping =
  exports.withRemoveFromGroupEntry =
  exports.withAddToGroupEntry =
  exports.withInitGroupStakeEntry =
  exports.withBoostStakeEntry =
  exports.withCloseStakeBooster =
  exports.withUpdateStakeBooster =
  exports.withInitStakeBooster =
  exports.withDoubleOrResetTotalStakeSeconds =
  exports.withReassignStakeEntry =
  exports.withCloseStakeEntry =
  exports.withCloseStakePool =
  exports.withReturnReceiptMint =
  exports.withUpdateTotalStakeSeconds =
  exports.withUpdateStakePool =
  exports.withUnstake =
  exports.withStake =
  exports.withClaimReceiptMint =
  exports.withInitStakeMint =
  exports.withDeauthorizeStakeEntry =
  exports.withAuthorizeStakeEntry =
  exports.withInitStakeEntry =
  exports.withInitStakePool =
  exports.withInitPoolIdentifier =
    void 0;
...
const withInitStakePool = async (transaction, connection, wallet, params) => {
  const identifierId = (0, pda_3.findIdentifierId)();
  const identifierData = await (0, common_1.tryGetAccount)(() =>
    (0, accounts_3.getPoolIdentifier)(connection)
  );
  const identifier =
    (identifierData === null || identifierData === void 0
      ? void 0
      : identifierData.parsed.count) || new anchor_1.BN(1);
  const program = (0, constants_1.stakePoolProgram)(connection, wallet);
  if (!identifierData) {
    const ix = await program.methods
      .initIdentifier()
...
const withInitStakeEntry = async (transaction, connection, wallet, params) => {
  const stakeEntryId = await (0, utils_2.findStakeEntryIdFromMint)(
    connection,
    wallet.publicKey,
    params.stakePoolId,
    params.originalMintId
  );
  const originalMintMetadatId = (0, common_1.findMintMetadataId)(
    params.originalMintId
  );
  const remainingAccounts = (0, utils_2.remainingAccountsForInitStakeEntry)(
    params.stakePoolId,
    params.originalMintId
  );
  const program = (0, constants_1.stakePoolProgram)(connection, wallet);
 ...
 * @returns Transaction
 */
const withAuthorizeStakeEntry = async (
  transaction,
  connection,
  wallet,
  params
) => {
  const stakeAuthorizationId = (0, pda_3.findStakeAuthorizationId)(
    params.stakePoolId,
    params.originalMintId
  );
  const program = (0, constants_1.stakePoolProgram)(connection, wallet);
 ...
 * @returns Transaction
 */
const withDeauthorizeStakeEntry = async (
  transaction,
  connection,
  wallet,
  params
...
const withInitStakeMint = async (transaction, connection, wallet, params) => {
  const [mintManagerId] = await (0, pda_1.findMintManagerId)(
    params.stakeMintKeypair.publicKey
  );
  const originalMintMetadataId = (0, common_1.findMintMetadataId)(
    params.originalMintId
  );
  const stakeMintMetadataId = (0, common_1.findMintMetadataId)(
    params.stakeMintKeypair.publicKey
  );
  const stakeEntryStakeMintTokenAccountId = await (0, common_1.findAta)(
    params.stakeMintKeypair.publicKey,
    params.stakeEntryId,
    true
  );
...
const withClaimReceiptMint = async (
  ...
  if (
    params.receiptType === constants_1.ReceiptType.Original &&
    (await (0, utils_1.getMintSupply)(connection, params.receiptMintId)).gt(
      new anchor_1.BN(1)
    )
  ) {
    throw new Error(
  ...
  const tokenManagerReceiptMintTokenAccountId = await (0,
  common_1.withFindOrInitAssociatedTokenAccount)(
    transaction,
    connection,
    params.receiptMintId,
    (
      await (0, pda_1.findTokenManagerAddress)(params.receiptMintId)
    )[0],
    wallet.publicKey,
    true
  );
  const stakeEntryReceiptMintTokenAccountId = await (0, common_1.findAta)(
    params.receiptMintId,
    params.stakeEntryId,
    true
  );
  const userReceiptMintTokenAccountId = await (0, common_1.findAta)(
    params.receiptMintId,
    wallet.publicKey,
    true
  );
  const [tokenManagerId] = await (0, pda_1.findTokenManagerAddress)(
    params.receiptMintId
  );
  const [mintCounterId] = await (0, pda_1.findMintCounterId)(
    params.receiptMintId
  );
  const remainingAccountsForKind = await (0,
  tokenManager_1.getRemainingAccountsForKind)(
    params.receiptMintId,
    params.receiptType === constants_1.ReceiptType.Original
...
const withStake = async (transaction, connection, wallet, params) => {
  ...
  const stakeEntryOriginalMintTokenAccountId = await (0,
  common_1.withFindOrInitAssociatedTokenAccount)(
    transaction,
    connection,
    params.originalMintId,
  ...
  const program = (0, constants_1.stakePoolProgram)(connection, wallet);
  const ix = await program.methods
    .stake(params.amount || new anchor_1.BN(1))
    .accounts({
      stakeEntry: stakeEntryId,
      stakePool: params.stakePoolId,
...
const withUnstake = async (transaction, connection, wallet, params) => {
  ...
  const [stakeEntryData, rewardDistributorData] = await Promise.all([
    (0, common_1.tryGetAccount)(() =>
      (0, accounts_3.getStakeEntry)(connection, stakeEntryId)
    ),
    (0, common_1.tryGetAccount)(() =>
      (0, accounts_2.getRewardDistributor)(connection, rewardDistributorId)
    ),
  ]);
  if (!stakeEntryData) throw "Stake entry not found";
  const stakePoolData = await (0, accounts_3.getStakePool)(
    connection,
    params.stakePoolId
  );
  if (
  ...
  const userOriginalMintTokenAccountId = await (0,
  common_1.withFindOrInitAssociatedTokenAccount)(
    transaction,
    connection,
    params.originalMintId,
  ...
  const remainingAccounts = await (0, utils_2.withRemainingAccountsForUnstake)(
    transaction,
    connection,
    wallet,
    stakeEntryId,
...
const withUpdateTotalStakeSeconds = async (
  transaction,
  connection,
  wallet,
  params
...
const withReturnReceiptMint = async (
  transaction,
  connection,
  wallet,
  params
  ...
  const stakeEntryData = await (0, common_1.tryGetAccount)(() =>
    (0, accounts_3.getStakeEntry)(connection, params.stakeEntryId)
  );
  if (!stakeEntryData) {
  ...
  const tokenManagerId = await (0, pda_1.tokenManagerAddressFromMint)(
    connection,
    receiptMint
  );
  ...
  tokenManager_1.withRemainingAccountsForReturn)(
    transaction,
    connection,
    wallet,
    tokenManagerData
  );
  const tokenManagerTokenAccountId = await (0, common_1.findAta)(
    receiptMint,
  ...
  const transferAccounts = await (0,
  tokenManager_1.getRemainingAccountsForKind)(
    receiptMint,
...
const withReassignStakeEntry = async (
  transaction,
  connection,
  wallet,
  params
...
const withDoubleOrResetTotalStakeSeconds = async (
  transaction,
  connection,
  wallet,
  params
...
const withInitStakeBooster = async (
  transaction,
  connection,
  wallet,
  params
) => {
  const stakeBoosterId = (0, pda_3.findStakeBoosterId)(
    params.stakePoolId,
...
const withUpdateStakeBooster = async (
  transaction,
  connection,
  wallet,
  params
...
const withCloseStakeBooster = async (
  transaction,
  connection,
  wallet,
  params
...
const withBoostStakeEntry = async (transaction, connection, wallet, params) => {
  ...
  const stakeBooster = await (0, accounts_3.getStakeBooster)(
    connection,
    stakeBoosterId
  );
  const paymentManager = await (0, accounts_1.getPaymentManager)(
    connection,
    stakeBooster.parsed.paymentManager
  );
  const feeCollectorTokenAccount = await (0,
  common_1.withFindOrInitAssociatedTokenAccount)(
    transaction,
    connection,
  ...
  const paymentRecipientTokenAccount = await (0,
  common_1.withFindOrInitAssociatedTokenAccount)(
    transaction,
    connection,
    stakeBooster.parsed.paymentMint,
 ...
 */
const withInitGroupStakeEntry = async (
  transaction,
  connection,
  wallet,
  params
 ...
 * @returns Transaction, public key for the created group stake entry
 */
const withRemoveFromGroupEntry = async (
  transaction,
  connection,
  wallet,
  params
...
const withClaimStakeEntryFunds = async (
  transaction,
  connection,
  wallet,
  stakeEntryId,
  ...
  const stakeEntryFundsMintTokenAccountId = (0,
  spl_token_1.getAssociatedTokenAddressSync)(fundsMintId, stakeEntryId, true);
  const userFundsMintTokenAccountId = await (0,
  common_1.withFindOrInitAssociatedTokenAccount)(
    transaction,
    connection,
    fundsMintId,
