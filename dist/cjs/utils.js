...
exports.calculatePendingGroupRewards =
...
const getMintSupply = async (connection, originalMintId) => {
  return new anchor_1.BN(
    (
      await (0, spl_token_1.getMint)(connection, originalMintId)
    ).supply.toString()
  );
};
...
const getPendingRewardsForPool = async (
  ...
  const rewardDistributorTokenAccountInfo = await (0, spl_token_1.getAccount)(
    connection,
    rewardDistributorTokenAccount
  );
  const stakeEntryIds = await Promise.all(
    mintIds.map(async (mintId) =>
      (0, utils_1.findStakeEntryIdFromMint)(
        connection,
        wallet,
  ...
  const rewardEntryIds = stakeEntryIds.map((stakeEntryId) =>
    (0, pda_1.findRewardEntryId)(rewardDistributor.pubkey, stakeEntryId)
  );
  ...
  return (0, exports.getRewardMap)(
    ...
    rewardEntries,
    rewardDistributor,
    new anchor_1.BN(rewardDistributorTokenAccountInfo.amount.toString()),
    UTCNow
  );
};
 ...
 * @returns
 */
const calculatePendingGroupRewards = (
  ...
  const rewardSecondsReceived =
    (groupRewardEntry === null || groupRewardEntry === void 0
      ? void 0
      : groupRewardEntry.parsed.rewardSecondsReceived) || new anchor_1.BN(0);
  const multiplier =
    ((_a =
      groupRewardEntry === null || groupRewardEntry === void 0
        ? void 0
      ...
      ? void 0
      : _a.multiplier) || new anchor_1.BN(1);
  let groupRewardSeconds = new anchor_1.BN(UTCNow).sub(
    groupEntry.parsed.changedAt
  );
  if (groupRewardDistributor.parsed.maxRewardSecondsReceived) {
    groupRewardSeconds = anchor_1.BN.min(
      groupRewardSeconds,
  ...
  let groupRewardAmountToReceive = groupRewardSeconds
    ...
    .div(
      new anchor_1.BN(10).pow(
        new anchor_1.BN(groupRewardDistributor.parsed.multiplierDecimals)
      )
    )
    .mul(groupRewardDistributor.parsed.baseMultiplier)
    .div(
      new anchor_1.BN(10).pow(
        new anchor_1.BN(groupRewardDistributor.parsed.baseMultiplierDecimals)
      )
    )
    .add(
      groupRewardDistributor.parsed.baseAdder.div(
        new anchor_1.BN(10).pow(
          new anchor_1.BN(groupRewardDistributor.parsed.baseAdderDecimals)
        )
      )
    );
  if (
  ...
  const nextRewardsIn = groupRewardDistributor.parsed.rewardDurationSeconds.sub(
    (groupEntry.parsed.groupCooldownStartSeconds || new anchor_1.BN(UTCNow))
